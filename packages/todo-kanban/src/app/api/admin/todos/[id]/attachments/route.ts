...
import { prisma, requireAdmin } from '@/lib/todo-adapters';

export const dynamic = 'force-dynamic';

/**
 * POST /api/admin/todos/[id]/attachments — enregistre un fichier joint
 * (le fichier lui-même est téléversé côté client via `uploadFile`).
 * Body : { name, url }
 */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Sécurité : n'accepte que des URLs https. Pour durcir, restreins au domaine de
  // ton stockage, ex. /^https:\/\/[a-z0-9]+\.supabase\.co\/storage\/v1\/object\/public\//
  if (!/^https:\/\//.test(url)) {
