...
export default function ConfirmationPage() {
  ...
  if (error) {
    return (
      <section className="py-24 px-4 text-center">
        <h1 className="font-cinzel-decorative text-2xl text-gradient-gold mb-4">
          Rendez-vous introuvable
        </h1>
        <p className="text-parchemin-vieilli/60 font-philosopher mb-8">
          Ce lien n'est pas valide ou le rendez-vous n'existe plus.
        </p>
        <Link
          href="/reserver"
          className="font-cinzel text-sm text-or-ancien hover:text-or-clair transition-colors tracking-wider"
        >
  ...
  const status = isCancelled ? statusLabels.cancelled : statusLabels[appointment.status] || statusLabels.pending;

  return (
    <section className="py-16 md:py-24 px-4">
      <div className="max-w-lg mx-auto text-center">
        <div className="text-5xl text-or-ancien mb-6 select-none animate-glow-pulse">
          {appointment.service.emoji}
        </div>

        <h1 className="font-cinzel-decorative text-2xl md:text-3xl text-gradient-gold mb-2">
          Votre Seance
        </h1>

        <p className={`font-cinzel text-sm tracking-wider mb-8 ${status.color}`}>
          {status.text}
        </p>
        ...
        <div className="bg-charbon-mystere border border-or-ancien/20 rounded-sm p-8 text-left mb-8">
          <div className="space-y-4 text-base font-cormorant">
            <div className="flex justify-between">
              <span className="text-parchemin-vieilli/60">Nom</span>
              <span className="text-parchemin">{appointment.clientName}</span>
            </div>
            <div className="h-px bg-violet-royal/15" />
            <div className="flex justify-between">
              <span className="text-parchemin-vieilli/60">Service</span>
              <span className="text-parchemin font-cinzel text-sm">{appointment.service.name}</span>
            </div>
            <div className="h-px bg-violet-royal/15" />
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-parchemin">
                {appointment.startTime.replace(':', 'h')} - {appointment.endTime.replace(':', 'h')}
              </span>
            </div>
            <div className="h-px bg-violet-royal/15" />
            <div className="flex justify-between">
              <span className="text-parchemin-vieilli/60">Duree</span>
              <span className="text-parchemin">{appointment.service.durationMinutes} min</span>
            </div>
