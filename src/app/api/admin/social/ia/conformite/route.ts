...
/**
 * POST /api/admin/social/ia/conformite — { postId } → rapport de conformité Meta.
 * Aide rédactionnelle : ne garantit PAS l'approbation par Meta.
 * Le rapport est persisté dans SocialPost.complianceReport.
 */
export async function POST(req: Request) {
  ...
  try {
    ...
    await prisma.socialPost.update({
      where: { id: post.id },
      data: {
        complianceReport: JSON.parse(JSON.stringify({ ...rapport, verifieLe: new Date().toISOString() })),
      },
    });
    return NextResponse.json(rapport);
  } catch (e) {
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Échec de la vérification.' },
      { status: 502 },
    );
  }
}
